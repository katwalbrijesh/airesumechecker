...
router.post("/create-checkout-session", requireAuth, asyncHandler(async (req, res) => {
  ...
  const clientUrl = process.env.CLIENT_ORIGIN.split(",")[0];
  ...
  const sessionConfig = {
    ...
    line_items: [{
      price_data: {
        currency: "usd",
        product_data: { name: "Resume Roaster Pro" },
        unit_amount: 1299,
...
router.post("/redeem-coupon", requireAuth, asyncHandler(async (req, res) => {
  const { code } = req.body;
  ...
  if (coupon.expiresAt && new Date() > coupon.expiresAt) throw ApiError.badRequest("This coupon has expired");
  if (coupon.usedCount >= coupon.maxUses) throw ApiError.badRequest("This coupon has already been fully used");
  ...
  res.json({ ok: true, type: "full", message: "Coupon applied! You are now on the Pro plan.", plan: "pro" });
}));
...
router.post("/webhook", express.raw({ type: "application/json" }), asyncHandler(async (req, res) => {
  ...
  switch (event.type) {
    case "checkout.session.completed": {
      ...
      await User.updateOne({ _id: session.metadata.userId }, { $set: { plan: "pro" } });
    ...
    case "customer.subscription.updated": {
      ...
      const isEnding = ["canceled", "unpaid", "incomplete_expired"].includes(subscription.status);
      ...
      console.log(`Subscription ${subscription.id} updated: status=${subscription.status}, cancelAtPeriodEnd=${subscription.cancel_at_period_end}`);
      break;
    }
...
router.get("/verify-session", requireAuth, asyncHandler(async (req, res) => {
  ...
  if (session.payment_status !== "paid" && session.status !== "complete") {
    return res.json({ ok: true, plan: req.user.plan, pending: true });
  }
  ...
  res.json({ ok: true, plan: "pro", user: updated });
}));
...
router.get("/subscription", requireAuth, asyncHandler(async (req, res) => {
  ...
  if (user.plan === "pro") {
    paymentStatus = user.subscriptionStatus === "active" ? "paid" : (user.subscriptionStatus || "unknown");
  }
